(function () {
    ...
    function photoUrl(folder, file) {
        return [PHOTO_ROOT, folder, file].map(function (part) {
    ...
    const cases = {
        seminars4: {
            ...
            body:
                "<p>Комплекс услуг в рамках проведения четырёх семинаров для федерального заказчика.</p><p>География охвата — четыре субъекта РФ; полный цикл организации и сопровождения в рамках контракта.</p>",
        ...
        seminars2: {
            ...
            body:
                "<p>Комплекс услуг в рамках проведения двух семинаров.</p><p>Реализация на площадках в двух регионах с единым стандартом качества для заказчика.</p>",
        ...
        tourism: {
            ...
            body:
                "<p>Организация и проведение заседания коллегии Госкомитета РТ по туризму «Об итогах работы в 2025 году и задачах на 2026 год».</p>",
    ...
    function descToHtml(text) {
        return text
            ...
            .map(function (p) {
                ...
                if (/^(Ключевые задачи|Проекты и заказчики|Среди заказчиков)$/i.test(chunk)) {
                ...
                const isList = lines.length > 1 && lines.every(function (l) {
    ...
    document.querySelectorAll(".hitbox").forEach(function (hb) {
        hb.addEventListener("mousemove", function (e) {
            const key = hb.getAttribute("data-member");
    ...
    if (galleryPrev) galleryPrev.addEventListener("click", function () { galleryStep(-1); });
    if (galleryNext) galleryNext.addEventListener("click", function () { galleryStep(1); });
